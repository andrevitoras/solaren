...
class PySolTrace:
    ...
    class Sun:
        ...
        user_intensity_table : [[float,float],]
            ...
            A typical table will have angles spanning 0->~5mrad, and intensities starting at 1 and decreasing to zero.
            The table must contain at least 2 entries.
        ...
        def Create(self, pdll, p_data):
            ...
            pdll.st_sun_xyz(c_void_p(p_data), c_number(self.position.x), c_number(self.position.y),
                            c_number(self.position.z))
            ...
            if len(self.user_intensity_table) > 2 and self.shape.lower()[0] == 'd':
                ...
                return pdll.st_sun_userdata(c_void_p(p_data), c_uint32(len(self.user_intensity_table)),
                                            pointer(user_angles), pointer(user_ints))
    ...
    class Stage:
        ...
        class Element:
            ...
            def Create(self, pdll, p_data) -> int:
                ...
                pdll.st_element_enabled(c_void_p(p_data), c_uint32(self.stage_id), c_uint32(self.id),
                                        c_int(int(self.enabled)));
                pdll.st_element_xyz(c_void_p(p_data), c_uint32(self.stage_id), c_uint32(self.id),
                                    c_number(self.position.x), c_number(self.position.y), c_number(self.position.z));
                pdll.st_element_aim(c_void_p(p_data), c_uint32(self.stage_id), c_uint32(self.id), c_number(self.aim.x),
                                    c_number(self.aim.y), c_number(self.aim.z));
                pdll.st_element_zrot(c_void_p(p_data), c_uint32(self.stage_id), c_uint32(self.id), c_number(self.zrot));
                pdll.st_element_aperture(c_void_p(p_data), c_uint32(self.stage_id), c_uint32(self.id),
                                         c_wchar(self.aperture[0]));
                pdll.st_element_aperture_params(c_void_p(p_data), c_uint32(self.stage_id), c_uint32(self.id),
                                                pointer(aperture_params));
                pdll.st_element_surface(c_void_p(p_data), c_uint32(self.stage_id), c_uint32(self.id),
                                        c_wchar(self.surface[0]));
                pdll.st_element_surface_params(c_void_p(p_data), c_uint32(self.stage_id), c_uint32(self.id),
                                               pointer(surface_params));
                if self.surface_file:
                    pdll.st_element_surface_file(c_void_p(p_data), c_uint32(self.stage_id), c_uint32(self.id),
                                                 c_char_p(self.surface_file.encode()));
                pdll.st_element_interaction(c_void_p(p_data), c_uint32(self.stage_id), c_uint32(self.id),
                                            c_int(self.interaction));  # /* 1=refract, 2=reflect */
                pdll.st_element_optic(c_void_p(p_data), c_uint32(self.stage_id), c_uint32(self.id),
                                      c_char_p(self.optic.name.encode()));
        ...
        def Create(self, pdll, p_data) -> int:
            ...
            pdll.st_stage_xyz(c_void_p(p_data), c_uint32(self.id), c_number(self.position.x), c_number(self.position.y),
                              c_number(self.position.z))
            pdll.st_stage_aim(c_void_p(p_data), c_uint32(self.id), c_number(self.aim.x), c_number(self.aim.y),
                              c_number(self.aim.z))
            pdll.st_stage_zrot(c_void_p(p_data), c_uint32(self.id), c_number(self.zrot))
            pdll.st_stage_flags(c_void_p(p_data), c_uint32(self.id), c_int(int(self.is_virtual)),
                                c_int(int(self.is_multihit)), c_int(int(self.is_tracethrough)))
    ...
    def __load_dll(self):
        ...
        if sys.platform == 'win32' or sys.platform == 'cygwin':
            ...
            pdll = CDLL('C:/SolTrace/3.4.0/api' + "/coretrace_api.dll")
            # print("Loaded win32")
    ...
    def run(self, seed: int = -1, as_power_tower=False, nthread=1, thread_id=0):
        ...
        **must** be called inside an import guard, e.g.:
        ...
        self.seed = seed
        self.cpus = nthread
        self.is_point_focus = as_power_tower
        ...
        if nthread in [0, 1]:
            ...
            pdll.st_sim_errors(c_void_p(p_data), c_int(1 if self.is_sunshape else 0),
                               c_int(1 if self.is_surface_errors else 0))
            ...
            self.powerperray = (self.sunstats['xmax'] - self.sunstats['xmin']) * (
                    self.sunstats['ymax'] - self.sunstats['ymin']) / self.sunstats['nsunrays'] * self.dni
    ...
    def __get_sun_stats(self, pdll, p_data):
        ...
        pdll.st_sun_stats(c_void_p(p_data), pointer(xmin), pointer(xmax), pointer(ymin), pointer(ymax),
                          pointer(nsunrays))
    ...
    def __get_ray_dataframe(self, pdll, p_data):
        """
        Get a pandas dataframe with all the ray data from the simulation.
    ...
    def element_raydata(self, element, absorbed_only=True):

        if self.raydata is None:
            raise RuntimeError("No ray data available! Run simulation first!")
        ...
        eu_angles = self.util_calc_euler_angles(numpy.array(element.position.as_list()),
                                                numpy.array(element.aim.as_list()), element.zrot)
    ...
    def element_stats(self,
                      ...
                      xbins: int,
                      ybins: int,
                      absorbed_only=True) -> dict:

        """
        This method was implemented to correspond to the 'elementstats' LK script function of SolTrace.
        ...
        dfr = self.element_raydata(element=element, absorbed_only=absorbed_only)
        nx = abs(xbins)
        ny = abs(ybins)

        if element.surface not in ['f', 't']:
            raise (RuntimeError(
                f"Surface type {element.surface} is not supported for flux plot generation. Must be one of 'f', 't'."))
        ...
        if element.surface == 'f':
            ...
            raybins_x = numpy.floor(
                (numpy.arctan2(dfr.loc_xt, dfr.loc_zt - D / 2) + math.pi) * nx / (2 * math.pi)).astype(int)
            raybins_y = numpy.floor((dfr.loc_yt + H / 2) / H * ny).astype(int)
            W = numpy.pi*D
            radius = D/2
        ...
        xbin_size = W / nx
        ybin_size = H / ny
        bin_area = xbin_size * ybin_size

        flux = numpy.zeros(shape=(nx, ny))
        for r in range(len(raybins_x)):
            flux[raybins_x.values[r], raybins_y.values[r]] += 1

        x_values = numpy.array([-W/2 + (0.5 + i)*xbin_size for i in range(nx)])
        y_values = numpy.array([-H/2 + (0.5 + i)*ybin_size for i in range(ny)])

        element_stats = {'dni': self.dni,
                         'generated_rays': self.sunstats['nsunrays'],
                         'power_per_ray': self.powerperray,
                         'num_rays': flux.sum(),
                         'avg_flux': flux.mean() * self.powerperray / bin_area,
                         'peak_flux': flux.max() * self.powerperray / bin_area,
                         'min_flux': flux.min() * self.powerperray / bin_area,
                         'radius': radius,
                         'bin_size_x': xbin_size,
                         'bin_size_y': ybin_size,
                         'xvalues': x_values,
                         'yvalues': y_values,
                         'flux': flux}
    ...
    def plot_flux(self,
        ...
        # Get a pandas dataframe with all the ray data
        df = self.raydata
        ...
        if element.surface == 'f':
            ...
            # x_rec = numpy.arange(-W/2, W/2, W / nx)
        ...
        plt.contourf(Yr, Xr, flux_st, levels=levels, cmap='Greys_r')
        plt.colorbar()
    ...
    def util_transform_to_local(self, posref: numpy.array, cosref: numpy.array, origin: numpy.array,
                                rreftoloc: numpy.array):
        """
    ...
    def util_transform_to_ref(self, posloc, cosloc, origin, rloctoref):
        ...
        cosref = numpy.dot(rloctoref, cosloc)
    ...
    def util_rotation_arbitrary(self, theta, axis, axloc, pt):
        ...
        fin.x = (a * (v * v + w * w) - u * (b * v + c * w - u * x - v * y - w * z)) * (1. - costh) + x * costh + (
                -c * v + b * w - w * y + v * z) * sinth
        fin.y = (b * (u * u + w * w) - v * (a * u + c * w - u * x - v * y - w * z)) * (1. - costh) + y * costh + (
                c * u - a * w + w * x - u * z) * sinth
        fin.z = (c * (u * u + v * v) - w * (a * u + b * v - u * x - v * y - w * z)) * (1. - costh) + z * costh + (
                -b * u + a * v - v * x + u * y) * sinth
    ...
    def util_calc_zrot_azel(self, vect) -> float:
        ...
        beta = -bsign * math.acos((math.pow(vect_i, 2) + math.pow(vect_k, 2)) /
                                  max(math.sqrt(math.pow(vect_i, 2) + math.pow(vect_k, 2)),
    ...
    def write_soltrace_input_file(self, path: str):
        ...
        with open(path, 'w') as fout:
            ...
            dt = datetime.now()
            fout.write(
                "# SOLTRACE VERSION 3.4 INPUT FILE\n")
                # "# SOLTRACE VERSION 3.4 INPUT FILE -- GENERATED BY soltrace-api v{:s} on {:02d}/{:02d}/{:04d} at {:02d}:{:02d}:{:02d}\n".format(
                #     "v000", dt.day, dt.month, dt.year, dt.hour, dt.minute, dt.second))

            # ------------------- Sun shape
            fout.write("SUN\tPTSRC\t{:d}\tSHAPE\t{:1s}\tSIGMA\t{:f}\tHALFWIDTH\t{:f}\n".format(0, self.sun.shape,
                                                                                               self.sun.sigma,
                                                                                               self.sun.sigma))
            fout.write("XYZ\t{:f}\t{:f}\t{:f}\tUSELDH\t{:d}\tLDH\t{:f}\t{:f}\t{:f}\n".format(self.sun.position.x,
                                                                                             self.sun.position.y,
                                                                                             self.sun.position.z, 0, 0.,
                                                                                             0., 0.))
            if (self.sun.shape == 'd'):
            ...
            for optics in self.optics:
                ...
                for opt in [optics.front, optics.back]:
                    fout.write(
                        "OPTICAL\t{:1s}\t{:d}\t{:d}\t{:d}\t{:f}\t{:f}\t{:f}\t{:f}\t{:f}\t{:f}\t{:f}\t{:f}\t{:f}\t{:f}\n".format(
                            opt.dist_type, 3, 1, 4, opt.reflectivity, opt.transmissivity, opt.slope_error,
                            opt.spec_error, 0., 0., 0., 0., 0., 0.)
                    )
            ...
            for stage in self.stages:
                fout.write(
                    "STAGE\tXYZ\t{:f}\t{:f}\t{:f}\tAIM\t{:f}\t{:f}\t{:f}\tZROT\t{:f}\tVIRTUAL\t{:d}\tMULTIHIT\t{:d}\tELEMENTS\t{:d}\tTRACETHROUGH\t{:d}\n".format(
                        stage.position.x, stage.position.y, stage.position.z, stage.aim.x, stage.aim.y, stage.aim.z,
                        stage.zrot, int(stage.is_virtual), int(stage.is_multihit), len(stage.elements),
                        int(stage.is_tracethrough))
                )
            ...
            fout.write("TRACE\tNRAY\t{:d}\tNSUN\t{:d}\tCPU\t{:d}\tSEED\t{:d}\tSUNSHAPE\t{:d}\tERRORS\t{:d}\tPTFOCUS\t{:d}".format(
                self.num_ray_hits, self.max_rays_traced, multiprocessing.cpu_count() - 2, self.seed,
                int(self.is_sunshape), int(self.is_surface_errors), int(self.is_point_focus)
            ))
